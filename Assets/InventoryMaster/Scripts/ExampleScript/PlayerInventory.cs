...
public class PlayerInventory : MonoBehaviour
{
    ...
    void EquipWeapon(Item item)
    {
        ...
        {
            var playerScript = GameObject.FindGameObjectWithTag("Player").GetComponent<MyCharacter>();
            //add the weapon if you unequip the weapon
            if (item.itemName == "한손검")
            {
                playerScript.equipOneHand();
            }
            else if(item.itemName == "양손검")
            {
                playerScript.equipTwoHand();
            }
            else if (item.itemName == "랜스")
            {
                playerScript.equipSpear();
            }
            else if (item.itemName == "활")
            {
                playerScript.equipBow();
            }
        }
    }
    ...
    public void OnConsumeItem(Item item)
    {
        for (int i = 0; i < item.itemAttributes.Count; i++)
        {
            ...
            {
                PlayerData.HPplus(item.itemAttributes[i].attributeValue);
            ...
            {
                PlayerData.MPplus(item.itemAttributes[i].attributeValue);
